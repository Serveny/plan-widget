...
export class CanvasTimeScaler {
  ...
  private drawBlock(
    ...
    this.ctx.fillText(
      text,
      xStartText + 3,
      row.yStart + row.height - 6,
      width
    )
  }

  private getRowsToDraw(
    row1Scale: TimeScale,
    row2Scale?: TimeScale,
    row3Scale?: TimeScale
  ): CanvasTsRow[] {
    ...
    if (row2Scale != null)
      rows.push(new CanvasTsRow(heightRow, heightRow, row2Scale))
    if (row3Scale != null)
      rows.push(new CanvasTsRow(heightRow * 2, heightRow, row3Scale))
    return rows
  }
