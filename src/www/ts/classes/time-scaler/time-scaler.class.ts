...
export class TimeScaler {
  ...
  private addResizeObserver(): void {
    ...
    new ResizeObserver(() => {
      console.log('resize', oldWidth, this.el.offsetWidth)
      this.rows.forEach(row => row.resize(oldWidth))
  ...
  private addMove(ev: MouseEvent): void {
    ...
    const mmHandler = (ev: MouseEvent): void => {
      ...
      this.rows.forEach(row => row.move(movePx))
      this.clickPosPx = ev.x
    }, muHandler = (): void => {
  ...
  // private zoom(evX: number): void {
  //   const scales = this.rowsFocus
  //     .getRowScales(this.cache.focusHorizonSec)
  //   if (this.row1.scale !== scales.row1) this.row1.zoom(evX)
  // }
